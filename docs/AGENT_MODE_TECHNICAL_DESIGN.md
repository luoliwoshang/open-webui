...
Agent 是与 Chat 平级的会话模式。两种模式共享登录用户、群组、组织、权限、审计和底层文件存储能力，但拥有独立的会话、消息协议、路由和前端状态模型。
...
6. Agent 文件下载由 Open WebUI 后端代理并执行会话权限检查，不向浏览器暴露七牛 API Key 或未经授权的上游文件 URL。
7. 默认开发环境可以继续使用 SQLite；生产环境推荐 PostgreSQL、Redis 和 S3 兼容对象存储，并将 Web、Worker、Scheduler 分开部署。
...
| 文件 | `models/files.py`、`routers/files.py`、`utils/access_control/files.py` | Agent 产出文件写入 Open WebUI 存储，并建立会话关联 |
...
flowchart LR
    ...
    AD --> QA[七牛 AgentAPI]
    W --> DB
    W --> STORE[(Local/S3 Object Storage)]
...
- **Worker**：领取 `queued/running` run，创建或复用上游 Session，消费事件流，持久化事件和文件，更新终态。
- **Scheduler（可选）**：扫描超时 run、回收孤儿锁、触发重试和文件清理。
- **PostgreSQL**：生产环境的会话、事件、权限快照、游标和审计数据主库。
- **Redis**：分布式锁、队列、实时广播和短期去重缓存。消息历史不只存 Redis。
- **对象存储**：建议 S3 兼容存储；本地开发可使用现有 local provider。

单机部署时可先运行 Web 和 Worker 两个进程；多副本部署必须启用共享数据库、Redis 和对象存储，并使用租约锁防止同一 run 被多个 Worker 同时消费。
...
list_session_files(upstream_session_id, run_id)
download_file(upstream_file_id) -> bytes/stream
...
- 文件列表、文件下载、文件删除及 `/mnt/session/outputs/` 的可见性语义。
...
| --- | --- |
| `id` | Open WebUI 文件 ID |
| `conversation_id`, `run_id` | 所属会话和生成 run |
| `upstream_file_id` | 七牛文件 ID |
| `storage_key` | Open WebUI 本地/S3 Key |
| `filename`, `mime_type`, `size_bytes` | 展示元数据 |
| `sha256` | 下载完整性校验 |
| `status` | `pending/ready/failed/deleting/deleted` |
| `created_at`, `deleted_at` | 生命周期 |

可选择同时在现有 `file` 表创建记录，但必须增加不可绕过的 `agent_artifact` 关联；仅依赖 `file.user_id` 或上游 URL 不足以表达会话边界。
...
5. Worker 收到每个 AgentAPI 事件后：校验 -> 去重 -> 写 `agent_event` -> 更新 run/conversation 状态 -> 处理文件 -> 广播 Socket.IO。
...
- 原始 payload 必须大小限制和敏感字段脱敏，超大输出转对象存储并在事件中保留引用。
...
`GET /api/v1/agent-artifacts/{id}/download` 必须按以下顺序检查：登录 -> artifact 存在且未删除 -> conversation 可读 -> artifact 的 conversation_id/run_id 关联一致 -> 从受控存储流式下载。不能接受客户端传入 `storage_key`、上游 URL 或只凭猜测的文件 ID 直接下载。

文件列表应由会话查询返回，展示文件名、MIME、大小、生成时间、状态和预览能力。支持预览的类型可复用现有 FileNav 组件；未知类型只提供下载。删除会话时先阻止新读取，取消/删除上游 Session，再异步删除本地 artifact，保留审计记录和软删除状态。
...
建议统一错误码：`AGENT_DISABLED`、`AGENT_FORBIDDEN`、`SESSION_NOT_FOUND`、`SESSION_CLOSED`、`RUN_CONFLICT`、`UPSTREAM_TIMEOUT`、`UPSTREAM_RATE_LIMITED`、`UPSTREAM_PROTOCOL_ERROR`、`ARTIFACT_NOT_READY`。返回给用户的 `error_message` 要可理解，内部 traceback 只进服务端日志。
...
页面初始化顺序：获取会话详情 -> 获取事件分页 -> 获取 artifact 列表 -> 连接 Socket.IO -> 使用最后 sequence 订阅增量。断线重连重复该过程，允许重复事件但由前端 event_id 去重。
...
- 本地历史事件和审计默认软删除/保留策略由组织合规要求决定，至少在 artifact 真正清理前保留关联信息。

### 11.3 文件

文件随 artifact 状态异步清理；删除会话不应在请求线程内阻塞等待大文件删除。清理任务必须按 conversation_id 过滤，不能按 filename 或用户目录模糊删除。
...
| 文件下载失败 | artifact 保持 pending/failed，可单独重新拉取，不重跑整个 Agent run |
...
5. 如果复用现有 `file` 表，新增关联表和后台校验，先双写/回填元数据，再切换 Agent 下载接口。
6. 灰度启用时按管理员、组织或用户白名单开启 Agent UI；关闭 Feature Flag 时保留已持久化数据和 Worker 清理能力。
...
- 文件安全：错误 artifact ID、跨会话 ID、被删除文件、管理员范围。
...
- 负载测试长输出、多事件、多文件和并发会话；验证事件分页、数据库索引和对象存储流式下载。
...
加入持久化 Worker、租约恢复、服务重启接管、取消/重试、文件下载到受控存储、artifact 列表和预览。
...
启用 PostgreSQL/Redis/对象存储部署，执行迁移演练、压测、安全测试和灰度 Feature Flag；确认监控指标、告警和数据保留策略后扩大范围。
...
- 文件下载成功率、对象存储占用、artifact pending/failed 数。
- 权限拒绝、跨会话访问尝试、管理员查看/下载审计事件。

日志使用 request_id、conversation_id、run_id、upstream_session_id（可脱敏）关联，禁止记录 API Key 和完整敏感 payload。生产环境应设置事件和原始 payload 的保留期限及对象存储生命周期规则。

## 17. 验收标准映射

需求中的 1~5 由独立入口、Agent CRUD、`access_grant` 和启停控制覆盖；6~9 由 conversation 快照、run/event 持久化、Worker 租约和重连补齐覆盖；10~11 由 artifact 关联和后端代理下载覆盖；12~13 由 Chat 管理范围复用和 owner 检查覆盖；14~15 由生命周期状态机、加密配置和脱敏审计覆盖；16 通过新增表、路由和 Feature Flag 保证现有 Chat 不改协议、不迁移数据。
